...
export default function QuickDrawGame({ socket, matchId, player1Id, player2Id, currentUserId }) {
  // ============================================================
  // STATE
  // ============================================================
    ...
    return (
      <Card className="max-w-lg mx-auto">
        <CardContent className="p-8 text-center">
          ...
          <p className="text-muted-foreground">Quick Draw match starting soon</p>
        </CardContent>
      </Card>
    )
  }
    ...
    return (
      <Card className="max-w-lg mx-auto">
        <CardContent className="p-8 text-center">
          ...
          <h2 className="text-2xl font-bold mb-2">{isWinner ? 'You Won!' : 'Game Over'}</h2>
          <p className="text-muted-foreground">The word was: <span className="font-bold text-foreground">{finalResult.word}</span></p>
        </CardContent>
      </Card>
    )
  }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4">
      ...
      <Card>
        <CardContent className="p-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            {isDrawer ? <Pencil className="h-4 w-4 text-primary" /> : <Eye className="h-4 w-4 text-primary" />}
      ...
      <Card>
        <CardContent className="p-2">
          <canvas
            ...
            className={`w-full border border-border rounded bg-white ${isDrawer ? 'cursor-crosshair' : 'cursor-default'}`}
          />
        </CardContent>
      </Card>
      ...
      {/* Guess feedback */}
      {guessMessage && (
        <p className="text-center text-sm text-danger font-medium">{guessMessage}</p>
      )}
    </div>
  )
}
